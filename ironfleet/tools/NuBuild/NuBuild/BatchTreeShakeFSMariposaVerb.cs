...
namespace NuBuild
{
    ...
    using System.IO;
    using System.Linq;
    class BatchTreeShakeFSMariposaVerb 
        : Verb
    {
        ...
        public BatchTreeShakeFSMariposaVerb(SourcePath batch_file)
        {
            ...
            foreach (string line in File.ReadAllLines(IronRootDirectory.PathTo(batch_file)))
            {
                ...
                string prohibited_id = words[1];

                IEnumerable<string> args = new ArraySegment<string>(words, 2, words.Length - 2);

                this.upstream.Add(new MariposaTreeShakeFSVerb(src, prohibited_id, args));
            }
